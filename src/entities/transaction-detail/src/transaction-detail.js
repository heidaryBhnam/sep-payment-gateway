module.exports = function buildMakeTransactionDetail({
  ...
  return function makeTransactionDetail({
    ...
    if (!RRN) throw new Error("makeTransactionDetail must have RRN.")

    if (!RefNum) throw new Error("makeTransactionDetail must have RefNum.")

    makeRefNum({ refNumValue: RefNum })

    if (!MaskedPan)
      throw new Error("makeTransactionDetail must have MaskedPan.")

    makeMaskedPan({ maskedPanValue: MaskedPan })
    ...
    else if (TerminalNumber != SEP_TERMINAL_ID)
      throw new Error(
        "makeTransactionDetail > TerminalNumber is not equal to setting SEP_TERMINAL_ID.",
      )
    ...
    if (!StraceNo) throw new Error("makeTransactionDetail must have StraceNo.")
  }
}
